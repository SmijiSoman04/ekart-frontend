...
export class ApiService {
  ...
  //using BehaviorSubject to pass stream of data from one component to another conmponent

  constructor(private http:HttpClient) {
    this.cartCount()
  }
  
  BASE_URL ='https://ekart-xq9o.onrender.com'
  //api function for get all products from database
    getAllProducts(){
...
//to get cart product count
    cartCount(){
      this.getCart().subscribe((result:any)=>{
        this.cartItemCount.next(result.length)//next is used because of behaviour subject
      })
    }
